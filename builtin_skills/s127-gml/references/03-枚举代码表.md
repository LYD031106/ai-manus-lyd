# 枚举代码表
由 `scripts/gen_s127_schema.py` 从 **S127.xsd**（version 1.0.0-20181129）生成，
共 51 个枚举属性、452 个取值。
XSD 变了就重跑生成脚本，不要手改。
...
经 13 个生产数据集 **882 处** `code="N"` 实测：**code == 该 label 在 XSD 枚举列表中的
1-based 序号**，吻合 882/882。本表据此生成。随时可复核：

```bash
python scripts/gen_s127_schema.py --verify-corpus '示例/**/*.gml'
```

> 若某个枚举在要素目录里被增删过一项，序号会整体错位。
> 换 XSD 或换要素目录版本后，**先跑一次上面的复核命令**再投产。
